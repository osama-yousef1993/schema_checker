...
class DatabaseCheckHandler():
    ...
    def tables_schema(self):
        try:
            ...
            # Check if the tables in the database match the tables in the schema file
    ...
    def function_procedure_schema(self):
        try:
            ...
            source_functions_procedure_list = list()
            destination_functions_procedure_list = list()
            destination_functions_procedure_type = list()
            destination_functions_procedure_matching = list()

            for statement in statements:
                if "CREATE OR REPLACE" in statement:
                    # get the type (Function, Procedure) and tha name for it from Schema file
